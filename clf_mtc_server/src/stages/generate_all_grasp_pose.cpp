...
GenerateAllGraspPose::GenerateAllGraspPose(const std::string& name)
...
void GenerateAllGraspPose::init(const moveit::core::RobotModelConstPtr& robot_model)
{
	...
	try { stages::GeneratePose::init(robot_model); }
	...
	for (double altitude = 0; altitude < 2.*M_PI; altitude += angle_delta) {
		for (double azimuth = 0; azimuth < 2.*M_PI; azimuth += angle_delta) {
			...
			for(double roll = 0; roll < 2.*M_PI; roll += M_PI / 8) {
				...
				Eigen::Quaterniond q;
				q = Eigen::AngleAxisd(roll, Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(altitude, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(azimuth, Eigen::Vector3d::UnitZ());
