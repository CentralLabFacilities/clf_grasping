...
std::multimap<float, geometry_msgs::PoseStamped> computeCylinderGrasps(const moveit_msgs::CollisionObject& object,
                                                                       const float MAX_GRIPPER_OPEN)
{
    ...
    for (uint8_t a = 0; a < CYLINDER_CONST1; ++a)
    {
      ...
      {  // TODO do this in fixed steps instead, e.g. every 2 cm
        ...
        {  // gripper 180 deg rotated
          ...
          grasp.normalize();
          tf2::Vector3 position(-(DIST_TO_OBJ + object.primitives[0].dimensions[1]), 0.0,
                                object.primitives[0].dimensions[0] / 3.0 * b * (c ? 1.0 : -1.0));
...
std::multimap<float, geometry_msgs::PoseStamped> computeBoxGrasps(const moveit_msgs::CollisionObject& object,
                                                                  const float MAX_GRIPPER_OPEN)
{
      ...
      {  // Each side can have two grasp poses (90 degrees rotated)
        ...
        {  // Two sides in each dimension: front/back, left/right, top/bottom
          ...
          {  // translation
            ...
            {  // gripper 180 deg rotated around x-axis
              geometry_msgs::PoseStamped target_pose_msg;
              target_pose_msg.header.frame_id = object.id;  //"base_footprint";
              tf2::Quaternion grasp;
              float alpha, beta;
              ...
              grasps.insert({ (std::abs(d) + 1.0) * object.primitives[0].dimensions[a] / beta,
                              target_pose_msg });  // Favour grasps in the middle, favour grasps on small sides
            }
          }
        }
      }
...
std::multimap<float, geometry_msgs::PoseStamped> computeSphereGrasps(const moveit_msgs::CollisionObject& object,
                                                                     const float MAX_GRIPPER_OPEN)
{
  ...
  if (2.0 * object.primitives[0].dimensions[0] <= MAX_GRIPPER_OPEN)
  {
    ...
    for (uint8_t a = 0; a < 20; ++a) {
      ...
      for (uint8_t b = 0; b < 6; ++b) {
        target_pose_msg.pose.orientation = tf2::toMsg(rotation * Eigen::AngleAxisd(b * 1.047197551,
    Eigen::Vector3d::UnitX())); target_pose_msg.pose.position = tf2::toMsg(vec);
    ...
    /*for (uint8_t a = 0; a < 200; ++a) {
      ...
      tf2::Quaternion orientation(std::sqrt(1.0-u1)*std::sin(2.0*M_PI*u2), std::sqrt(1.0-u1)*std::cos(2.0*M_PI*u2),
    std::sqrt(u1)*std::sin(2.0*M_PI*u3), std::sqrt(u1)*std::cos(2.0*M_PI*u3)); // Random quaternion. Source:
    http://planning.cs.uiuc.edu/node198.html
      ...
      if(object.header.frame_id == "base_footprint") {
        ...
        position += tf2::Vector3(object.primitive_poses[0].position.x, object.primitive_poses[0].position.y,
    object.primitive_poses[0].position.z); } else { target_pose_msg.header.frame_id = object.id;
      }
    ...
    for (uint8_t a = 0; a < SPHERE_CONST1 / 2; ++a)
    {
      ...
      {
        for (uint8_t c = 0; c < SPHERE_CONST2; ++c)
        {  // rotations around x-axis of gripper
          geometry_msgs::PoseStamped target_pose_msg;
          tf2::Quaternion orientation;
          orientation.setRPY(2.0 * M_PI / SPHERE_CONST2 * c, 2.0 * M_PI / SPHERE_CONST1 * b,
                             2.0 * M_PI / SPHERE_CONST1 * a);
