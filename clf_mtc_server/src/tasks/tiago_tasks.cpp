...
Task TiagoTasks::createPickTask(const std::string& id, const std::string& support_id)
{
  ...
  if (!gripper_fingers.empty())
  {
...

    {
      ...
      allow_gripper_touch_support->setCallback(
          [support_id, gripper_fingers](const planning_scene::PlanningScenePtr& scene, const PropertyMap& p) {
...
moveit::task_constructor::Task TiagoTasks::createPlaceTask(const std::string& surface, const std::string& object_id,
